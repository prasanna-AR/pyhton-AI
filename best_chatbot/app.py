...
def get_system_prompt(mode, category=None):
    mode_text = get_mode_text(mode)
    
    category_instruction = ""
    if category == "Class 1-5":
        category_instruction = "The user is a young student in Class 1-5. Explain things VERY simply using basic words, fun analogies, and short sentences suitable for young children."
    elif category == "Class 6-12":
        category_instruction = "The user is a middle/high school student in Class 6-12. Explain things step-by-step, engagingly, suitable for teenagers."
    elif category == "College":
        category_instruction = "The user is a college student. Provide detailed, academic, and comprehensive explanations suitable for higher education."
    elif category == "Others":
        category_instruction = "The user is a general learner. Provide clear, straightforward, and engaging explanations."
    else:
        category_instruction = "The user is a general learner. Adapt your response to be clear and engaging."
...
- Target Audience: {category_instruction}

{mode_text}"""

def build_prompt(user_input, context, mode, category=None):
    rules = get_system_prompt(mode, category)
    return f"{rules}\n\nContext:\n{context}\n\nQuestion:\n{user_input}"
...
def select_category():
    if request.method == 'POST':
        ...
        if category in ["Class 1-5", "Class 6-12", "College", "Others"]:
...
def ask():
    ...
    category = session.get('category', 'Others')
    rules = get_system_prompt(mode, category)
    prompt_text = f"{rules}\n\nQuestion:\n{user_input}"
    ...
    if vectorstore is not None and user_input.strip():
        ...
        prompt_text = build_prompt(user_input, context, mode, category)
...
def edit_msg(msg_id):
    ...
    if vectorstore is None:
        ...
        context = "\n".join([d.page_content for d in docs])
        prompt = build_prompt(new_input, context, mode, category)
        history.append(HumanMessage(content=prompt))
...
@app.route('/api/flashcards/<int:convo_id>')
@login_required
def get_flashcards(convo_id):
    convo = Conversation.query.filter_by(id=convo_id, user_id=session['user_id']).first()
    if not convo:
    ...
    for msg in messages[-10:]:
        clean_user_input = re.sub('<[^<]+>', '', msg.user_input)
        clean_bot_response = re.sub('<[^<]+>', '', msg.bot_response)
        context += f"Q: {clean_user_input}\nA: {clean_bot_response}\n\n"
        
    prompt = f"""Based on the following conversation, generate 10 highly concise flashcards for revision. 
Return ONLY a valid JSON array of objects, where each object has 'q' (the question/concept) and 'a' (a concise 1-2 sentence answer). 
Do NOT wrap it in markdown block. Just the raw JSON array. Example: [{{"q": "Q1", "a": "A1"}}]
